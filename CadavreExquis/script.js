...
function setup() {
  ...
  socket.addEventListener("message", (event) => {
    ...
    // ▶️ Sound abspielen, wenn Nachricht empfangen
    if (type === "play-sound") {
      const soundType = msg[1];
      if (soundType === "reveal") {
        const audioElem = document.getElementById("revealSound");
        if (audioElem) {
          audioElem.play().catch(err => console.warn("Autoplay blockiert:", err));
        }
      }
    }
  });
